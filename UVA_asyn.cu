...
int main(int argc, char const *argv[]) {
	...
	for (int i = 0; i < rowy * coly; i++)
		B[i] = rand() % 100 + 1;
    cudaMemPrefetchAsync(A, sizeof(int) * rowx * colx, deviceId);
    cudaMemPrefetchAsync(B, sizeof(int) * rowx * colx, deviceId);
    cudaMemPrefetchAsync(C, sizeof(int) * rowx * colx, deviceId);
